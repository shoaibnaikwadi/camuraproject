...
def google_feed(request):
    ...
    for combo in ComboProduct.objects.all():
        ...
        components = (
            f"Camera: {combo.camera.name} x {combo.camera_qty}, "
            f"DVR: {combo.dvr.name}, "
            f"Hard Disk: {combo.hard_disk.name if combo.hard_disk else 'No HDD'}, "
            f"Cable: {combo.cable.name} x {combo.cable_qty}, "
            f"Power Supply: {combo.power.name} x {combo.power_qty}, "
        ...
        link = f"https://yourdomain.com/product/{combo.id}/"

        # Image
        image = (
            combo.image.url
            if combo.image
            else "https://yourdomain.com/static/no-image.jpg"
        )
